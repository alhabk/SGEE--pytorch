...
def main():
    ...
    if args.mode == 'test':
        ...
        print('         填满buffer            ')
        print('##############################')
        while len(agent.replay_buffer.storage_g) <= args.capacity_g:
            print('容量为：',len(agent.replay_buffer.storage_g))
            state = env.reset()
            for t in count():
                action = agent.select_action(state)
                ...
                agent.replay_buffer.storage_g.append((state, next_state, action, reward, np.float(done)))
                state = next_state
                if done:
                    break
        args.real_lenth = len(agent.replay_buffer.storage_g)
        print('##############################')
        print('            开始训练            ')
        print('##############################')
        if args.load:
